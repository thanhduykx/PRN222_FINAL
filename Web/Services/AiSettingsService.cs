...
public sealed class AiSettingsService : IAiSettingsService
{
    ...
    public AiSettingsService(IWebHostEnvironment environment, GeminiOptions gemini, FlmSyllabusAwareTextChunker chunker)
    {
        _gemini = gemini;
        _chunker = chunker;
    ...
    public async Task SaveAsync(AiSettings settings, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            {
                await JsonSerializer.SerializeAsync(stream, normalized,
    ...
    private void LoadSavedSettings()
    {
        ...
        try
        {
            var saved = JsonSerializer.Deserialize<AiSettings>(File.ReadAllText(_settingsPath));
